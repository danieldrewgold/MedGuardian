...
export default function HomeScreen({ navigation }: any) {
  ...
  const buildInteractionReport = () => {
    ...
    text += `Medications reviewed: ${medications.length}\n\n`;

    text += `CURRENT MEDICATIONS:\n`;
    medications.forEach((med, i) => {
      text += `  ${i + 1}. ${med.name} ${med.dosage} (${med.frequency})`;
      if (med.reason) text += ` — ${med.reason}`;
      text += '\n';
    });
    text += '\n';
    ...
    if (allergies.length > 0) {
      ...
      if (allergyConflicts.length > 0) {
        text += `ALLERGY CONFLICTS:\n`;
        allergyConflicts.forEach((c) => {
          text += `  ⚠ ${c.medication} — patient allergic to ${c.allergy}\n`;
        });
      }
      text += '\n';
    }

    text += `INTERACTIONS IDENTIFIED: ${interactions.length}\n`;
    text += `${'-'.repeat(50)}\n`;
    if (interactions.length === 0) {
      ...
      if (major.length > 0) {
        text += `\nMAJOR INTERACTIONS (${major.length}):\n`;
        major.forEach((ix) => {
          text += `  • ${ix.med1} + ${ix.med2}\n`;
          text += `    ${ix.description}\n`;
          if (ix.info) text += `    Note: ${ix.info}\n`;
          text += '\n';
        });
      }
    ...
    text += `${'-'.repeat(50)}\n`;
    text += `Generated by MedGuardian | ${new Date().toLocaleString()}\n`;
    text += `Always verify with clinical references.\n`;
    return text;
  };
  ...
  const handleShareAction = async (action: 'share' | 'copy') => {
    ...
    const fileName = shareMode === 'clinical'
      ...
      : shareMode === 'interactions'
      ? `interaction-report-${safeName}-${dateSuffix}.txt`
      ...
      >
        ...
        <View style={styles.shareSheet}>
          ...
          <Text style={styles.shareModeDesc}>
            {shareMode === 'clinical'
              ? `Full reconciliation with interactions & encounter notes`
              : shareMode === 'interactions'
              ? `Focused interaction report — ${interactions.length} interaction${interactions.length !== 1 ? 's' : ''} found`
